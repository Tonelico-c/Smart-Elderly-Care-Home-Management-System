...
<template>
  ...
  <el-table :data="list" style="width: 100%; margin-bottom: 20px" row-key="id" border>
    ...
    <el-table-column label="操作" align="center" width="200px" fixed="right" #default="{row}">
      <el-button size="small" type="success" @click="showAddDialog(row)" :disabled="row.type === 2">添加</el-button>
  ...
  <el-dialog v-model="dialogFormVisible" :title="title" width="500" :lock-scroll="false">
    <el-form :model="permission" label-width="100px" >
      <el-form-item label="上级权限">
        <el-input v-model="permission.parentName" :disabled="true"></el-input>
      </el-form-item>
      <el-form-item label="上级权限ID">
        <el-input v-model="permission.parentId" :disabled="true"></el-input>
      </el-form-item>
      ...
      <el-form-item label="权限编号ID">
        <el-input v-model="permission.id" placeholder="应以上级权限ID开头(0不用写)"></el-input>
      </el-form-item>
